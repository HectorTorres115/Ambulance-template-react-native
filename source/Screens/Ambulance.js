...
export const Ambulance = ({navigation}) => {
    //State
    ...
    const [ClientChats] = useLazyQuery(CLIENT_CHATS, {
        ...
        onCompleted: (data) => {
            ...
            if(data.GetClientChat == null){
                CreateChat({variables: {client: usuario.username, type: "Ambulance"}})
            }
        },
        onError: (error) => {
            console.log(error);
    ...
    useEffect(() => {
        console.log('Did mount screen: Ambulance');
        ClientChats({variables: {type: "Ambulance", client: usuario.username}})
